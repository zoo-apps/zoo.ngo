...
function Ourefforts() {
  ...
  return (
    <div className="bg-background xl:py-52 md:py-32 max-md:py-32 2xl:px-52 xl:px-32 lg:px-24 md:px-8 max-md:px-4">
        <p className='text-gray-300 text-md xl:text-2xl text-center'>Over 38,000 endangered species</p>
        <h1 className='text-foreground text-center md:text-2xl xl:text-5xl max-md:text-xl mt-8 mb-12 2xl:px-48 xl:px-32 lg:px-32 md:px-16'>We&apos;ve started initiating efforts with 7 species to raise awareness, secure funding, and strive for their conservation.</h1>
      ...
      {efforts.map((data, index) => (
        ...
        <Link href={data.url} className='text-foreground text-2xl pb-4 max-md:text-2xl max-md:hidden md:block lg:hidden text-center'>{data.title}</Link>
        <div className='flex max-md:hidden items-center text-foreground lg:pt-16 md:pt-4 max-md:pt-16 xl:space-x-16 lg::space-x-8 md:space-x-4 xl:mx-12 lg:mx-6 md:mx-4'>
            <Image
            ...
            <div className='flex flex-col w-2/3'>
                <Link href={data.url} className='text-foreground text-2xl pb-4 max-md:text-2xl md:hidden lg:block max-md:block'>{data.title}</Link>
                <p className='text-foreground max-md:text-lg lg:text-md md:text-sm'>{data.content}</p>
            </div>
        </div>
        <div className='max-md:flex flex-col hidden pt-12'>
            ...
            <Image
                ...
                alt=''
            />
            <Link href={data.url} className='w-4/5 text-foreground text-3xl'>{data.title}</Link>
            </div>
            <p className='text-foreground text-lg pt-5'>{data.content}</p>
        </div>
        </div>
      ))}
      </div>
